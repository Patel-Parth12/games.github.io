let game_arr = [];
let gamePlayer = true;
const onPlayerTrun = async function (i) {
  if (!gamePlayer) {
    return false;
  }
  if (isPositionisfree(i)) {
    ...
    if (computermove) {
      await waitingComputerMove("I'm Thinking");
...
const waitingComputerMove = async function (msg) {
  const myPromise = new Promise((resolve, reject) => {
    let roundCount = 0;
    let localMessage = msg;
    let emojiArray = [
      "🤔",
      "🧐",
      "😕",
    ...
    let intervalID = setInterval(() => {
      if (roundCount >= 5) {
        ...
        roundCount++;
        if (msg) {
          setMessage(localMessage + " " + getRandomItem(emojiArray));
        }
      }
    }, 300);
  });
...
const computerMove = function (turn) {
  ...
  let freePosition;
  //console.log(getCornerFreePosition(1,2,3,4,5,6,7,8,9).length,isMiddaleBoxisfree(oppTurn));
  freePosition = getCornerFreePosition(5);
  if (freePosition.length > 0) {
    return getRandomItem(freePosition);
  }
  if (
    getCornerFreePosition(1, 2, 3, 4, 5, 6, 7, 8, 9).length === 8 &&
    !isMiddaleBoxisfree(oppTurn)
  ) {
    freePosition = getCornerFreePosition(2, 4, 6, 8);
    //console.log(freePosition);
    if (freePosition.length > 0) {
      if (!freePosition.includes(2) && freePosition.includes(8)) {
        return 8;
      } else if (!freePosition.includes(8) && freePosition.includes(2)) {
        return 2;
      } else if (!freePosition.includes(4) && freePosition.includes(6)) {
        return 6;
      } else if (!freePosition.includes(6) && freePosition.includes(4)) {
        return 4;
      } else {
        return getRandomItem(freePosition);
      }
...
const isMiddaleBoxisfree = function (turn) {
  let retValue = true;
  let char = getPlayerChar(turn);
...
const winingCheck = function (gameBord, ch, a, b, c, isdummy) {
  let isWining =
    ch === gameBord[a] &&
    gameBord[a] === gameBord[b] &&
    gameBord[b] === gameBord[c];
  if (isWining && !isdummy) {
